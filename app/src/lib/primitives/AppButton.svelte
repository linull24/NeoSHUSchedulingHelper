...
<script lang="ts">
	...
	let {
		...
		loading = false,
		class: className = '',
		type: buttonType = 'button' as 'button' | 'submit' | 'reset',
		disabled = false
	} = $props();
	...
	const sizeMap: Record<Size, { regular: string; icon: string }> = {
		sm: {
			regular: 'h-8 px-3 text-[var(--app-text-sm)] gap-1.5',
		...
		lg: {
			regular: 'h-11 px-5 text-[var(--app-text-md)] gap-2.5',
			icon: 'h-11 w-11 text-[var(--app-text-md)]'
		}
	};
