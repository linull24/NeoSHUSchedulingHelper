...
## Definition of Done
- `rg '<style' app/src/lib app/src/routes`（允许 `node_modules` 之外任意路径）不得命中任何 `.svelte` 文件；所有样式必须集中在 `$lib/styles/**` 或共享 CSS 模块。
- Apps/components 不再 `@use "$lib/styles/apps/*.styles.scss"`；残留即视作未完成。
