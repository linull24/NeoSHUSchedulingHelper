...
- **Styling:** `app/src/lib/styles/components/pagination-footer.scss` applies tokenized spacing, adds a 360px column fallback, and hides page numbers/jump/total below 256px to match `spec://cluster/ui-templates#chunk-02`.
...
- **Rendering logic:** `CourseCalendarPanel.svelte:57-92` inspects each derived `visibleEntries` item. If `shouldShowLabel(entry)` (see below) returns false yet `getClipPath(entry)` is not `none`, the block renders a `.course-indicator` div with `String.fromCharCode(9312 + index % 20)` to display circled numerals while maintaining accessible `aria-label`s.
...
- **Styles:** `course-calendar-panel.scss:148-181` differentiates `.course-block.with-clip` cards, adds `.course-indicator` typography, and ensures highlighted/ghost states respect the new indicator.
...
- **Consumers:** `CourseCalendarPanel.svelte:57-92` derives `activeId` from the hover store to append `highlighted` classes, while `CourseCard.svelte:5-78` subscribes to `$hoveredCourse` and toggles `isHighlighted` when a calendar hover targets the same id. Styles in `course-card.scss:453-478` and `course-calendar-panel.scss:157-166` outline/halo the highlighted elements so users can track matches in both directions.
